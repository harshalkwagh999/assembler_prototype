...
{  
	...
	op_tab=(OPTAB *)malloc(12*sizeof(OPTAB));//allocating memory for optab
	...
	{
		tokenize(buffer,&tokenarr,&tokencnt);//call to tokenizer to create tokens

		optab_Initialization(op_tab,tokenarr);//call to optab initialization
	...
	// Actual Input File Parsing Starts Here
	...
	while(getline(&buffer,(size_t*)&n,inputf)!=(-1))
	{
		...
		switch(sec_flag)
		{
			...
			case 2:  // text section
				...
				{
					continue;    // control goes to read next line
				}
				switch(tokencnt)
				{
					case 1:   // only label
						...
						{
							lable=(char*)malloc(sizeof(char)*(strlen(tokenarr[0])));//allocating size for lable
					...
					case 3: // Unary instruction with lable OR Binary instruction
						...
						else//Binary instruction
						{
							...
							if(strcmp(mode1+1,mode2+1)!=0)// Ex: mode1=r32 & mode2=m32 (if both mode are not same size 32):error
							{
						...
						break;
					case 4:  // Binary instruction with lable
	...
	while(getline(&buffer,(size_t*)&n,interf)!=(-1))//read intermediate file
	{
		...
		if(tokencnt==3)//means binary instruction
		{
			...
			if(tokenarr[1][0]=='R' && tokenarr[2][0]=='R')//check for operand-1 & operand-2 are registers
			{
			...
			else if(tokenarr[1][0]=='R' && tokenarr[2][0]=='S')//check for operand-1 is register & operand-2 is symbol
			{
			...
			else if(tokenarr[1][0]=='R' && tokenarr[2][0]=='L')//check for operand-1 is register & operand-2 is literal
			{
				oprnd=(char*)malloc(sizeof(char)*20);//alocate mem for oprnd
				sprintf(str,"%d",bin_dec(tmp1[0]));//convert operand-1 to decimal formate and store in str(as per the nasm formate)
			...
			else if(tokenarr[1][0]=='S' && tokenarr[2][0]=='L')////check for operand-1 is symbol & operand-2 is literal
			{
				oprnd=(char*)malloc(sizeof(char)*20);//alocate mem for oprnd
				strcpy(oprnd,"[");//stote '['in oprnd
				sprintf(str,"%X",atoi(tmp1[0]));//convert oprand-1 to integer and store in hex formate
				strcat(oprnd,str);//append str to oprnd
				strcat(oprnd,"]");//append ']' to oprnd 
				sprintf(str,"%X",atoi(tmp1[1]));//convert oprand-2 to integer and store in hex formate
		...
		{
			tmp1=(char**)malloc(sizeof(char*)*2);//allocate mem for tmp1 2D string array
			...
			if(tyni_tokenarr[0][0]=='R')//if register
			{
				...
				{
					case 7:// NOT
						tmp1[0]=(char*)malloc(sizeof(char)*strlen(reg_tab[atoi(tyni_tokenarr[1])].RR_RI));//allocate mem for tmp1[0] RR_RI
						...
						oprnd=(char*)malloc(sizeof(char)*10);//alloacate mem foe oprnd
					...
					case 9:// DEC
						...
						tmp1[0]=(char*)malloc(sizeof(char)*strlen(reg_tab[atoi(tyni_tokenarr[1])].RR_RI));//allocate mem for RR_RI
					...
					case 11:// DIV
						tmp1[0]=(char*)malloc(sizeof(char)*strlen(reg_tab[atoi(tyni_tokenarr[1])].RR_RI));
						strcpy(tmp1[0],reg_tab[atoi(tyni_tokenarr[1])].RR_RI);
						tmp1[1]=(char*)malloc(sizeof(char)*strlen(reg_tab[6].RR_RI)); // (Implicit)source = DH register
